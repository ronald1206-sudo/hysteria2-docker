...
# 2. 建立目錄並寫入 sing-box VLESS+WS 配置文件 (採用官方最新統一的 transport 欄位)
RUN mkdir -p /etc/sing-box && echo '{"log":{"level":"warn"},"inbounds":[{"type":"vless","listen":"127.0.0.1","listen_port":10001,"users":[{"uuid":"66666666-6666-6666-6666-666666666666"}],"transport":{"type":"ws","path":"/ronald-vless"}}],"outbounds":[{"type":"direct"}]}' > /etc/sing-box/config.json
